...
class movegen {
  public:
    ...
    static constexpr int PUSH_OFFSET = 8;
     ...
     *
     * @tparam side_to_move boolean indicating side to move -- 0 = white; 1 = black -- used to
     * reduce low level branching
     *
     * @param b pointer to board representation
     * @param l pointer to movelist
     */
    template <bool side_to_move> static void generate_pawn_qmoves(bitboard *b, movelist *l) {
        uint8_t target;
        ...
        uint64_t double_pushes =
            (side_to_move) ? (((pushes & single_push_rank) >> PUSH_OFFSET) & ~b->occupancy[2])
        ...
        while (pushes != 0ULL) {
            ...
            l->moves[l->size].m = bit_move((uint8_t)target - push_offset, (uint8_t)target,
                                           bit_move::quiet_move, PAWN, EMPTY);
            l->size++;
            pushes = reset_lsb(pushes);
        }
        while (double_pushes != 0ULL) {
            target = BitScanForward64(double_pushes);
            l->moves[l->size].m = bit_move((uint8_t)target - double_push_offset, (uint8_t)target,
                                           bit_move::double_pawn_push, PAWN, EMPTY);
            l->size++;
     ...
     *
     * \tparam side_to_move boolean indicating side to move -- 0 = white; 1 = black -- used to
     * reduce low level branching
     *
     * \param b pointer to the board representation
     * \param l pointer to the movelist
     */
    template <bool side_to_move> static void generate_pawn_cmoves(bitboard *b, movelist *l) {
        uint8_t origin;
        uint8_t target;
        // getting pawn bitboard of side to move
        uint64_t pawns = b->bbs[PAWN][side_to_move];
        // generating a bitboard of push targets
        const uint64_t pushes = (side_to_move) ? ((pawns >> PUSH_OFFSET) & ~b->occupancy[2])
                                               : ((pawns << PUSH_OFFSET) & ~b->occupancy[2]);
        // getting the rank of promotion -> 8th rank when white is to move, 1st rank when black is
        // to move
        ...
        while (pawns != 0ULL) {
            ...
            pawn_attacks =
                attacks::pawn_attacks[side_to_move][origin] & (b->occupancy[!side_to_move]);
            ...
            while (pawn_attacks != 0ULL) {
                ...
                // the target is the en passant target square => the current capture is an en
                // passant capture
                if ((1ULL << target) == b->ep_target_square) {
                    // add the move to the movelist
                    l->moves[l->size].m = bit_move((uint8_t)origin, (uint8_t)target,
                                                   bit_move::ep_capture, PAWN, PAWN);
                    // increment movelist size counter
                    l->size++;
                }
                // if the capture is a promotion capture ...
                else if ((1ULL << target) & promotion_rank) {
                    // [the captured piece cannot be a pawn; pawn only exist between the 2nd and 7th
                    // rank] add all moves (one for each piece type the pawn can promote to) to the
                    // movelist
                    l->moves[l->size].m =
                        bit_move((uint8_t)origin, (uint8_t)target,
                                 bit_move::queen_capture_promotion, PAWN, capture_type);
                    l->moves[l->size + 1].m =
                        bit_move((uint8_t)origin, (uint8_t)target, bit_move::rook_capture_promotion,
                                 PAWN, capture_type);
                    l->moves[l->size + 2].m =
                        bit_move((uint8_t)origin, (uint8_t)target,
                                 bit_move::bishop_capture_promotion, PAWN, capture_type);
                    l->moves[l->size + 3].m =
                        bit_move((uint8_t)origin, (uint8_t)target,
                                 bit_move::knight_capture_promotion, PAWN, capture_type);
                    // increment movelist size counter
                    l->size += 4;
                }
                // move is a "standard" capture
                    ...
                    l->moves[l->size].m = bit_move((uint8_t)origin, (uint8_t)target,
                                                   bit_move::capture, PAWN, capture_type);
                    // increment movelist size counter
                    l->size++;
                }
        ...
        while (promotion_pushes != 0ULL) {
            ...
            l->moves[l->size].m = bit_move((uint8_t)target - offset, (uint8_t)target,
                                           bit_move::queen_promotion, PAWN, EMPTY);
            l->moves[l->size + 1].m = bit_move((uint8_t)target - offset, (uint8_t)target,
                                               bit_move::rook_promotion, PAWN, EMPTY);
            l->moves[l->size + 2].m = bit_move((uint8_t)target - offset, (uint8_t)target,
                                               bit_move::bishop_promotion, PAWN, EMPTY);
            l->moves[l->size + 3].m = bit_move((uint8_t)target - offset, (uint8_t)target,
                                               bit_move::knight_promotion, PAWN, EMPTY);
            // increment movelist size counter
            l->size += 4;
     ...
     */
    template <bool side_to_move> static void generate_knight_qmoves(bitboard *b, movelist *l) {
        ...
        while (knights != 0ULL) {
            ...
            while (knight_attacks != 0ULL) {
                ...
                l->moves[l->size].m =
                    bit_move((uint8_t)origin, (uint8_t)target, bit_move::quiet_move, KNIGHT, EMPTY);
                l->size++;
    ...
    /**
     * @brief Generates all knight captures.
     *
     * @tparam side_to_move boolean indicating side to move
     * @param b pointer to the board representation
     * @param l pointer to the movelist
     */
    template <bool side_to_move> static void generate_knight_cmoves(bitboard *b, movelist *l) {
        uint64_t knights = b->bbs[KNIGHT][side_to_move];
        ...
        while (knights != 0ULL) {
            ...
            while (knight_attacks != 0ULL) {
                target = BitScanForward64(knight_attacks);
                capture_type = b->piece_type_from_index(target);
                l->moves[l->size].m = bit_move((uint8_t)origin, (uint8_t)target, bit_move::capture,
                                               KNIGHT, capture_type);
                l->size++;
                knight_attacks = reset_lsb(knight_attacks);
            }
    ...
    /**
     * @brief Generates all quiet bishop moves. (no captures)
     *
     * @tparam side_to_move boolean indicating side to move
     * @param b pointer to the board representation
     * @param l pointer to the movelist
     */
    template <bool side_to_move> static void generate_bishop_qmoves(bitboard *b, movelist *l) {
        ...
        while (bishops != 0ULL) {
            ...
            bishop_attacks =
                attacks::get_bishop_attacks(origin, b->occupancy[2]) & (~b->occupancy[2]);
            while (bishop_attacks != 0ULL) {
                target = BitScanForward64(bishop_attacks);
                l->moves[l->size].m =
                    bit_move((uint8_t)origin, (uint8_t)target, bit_move::quiet_move, BISHOP, EMPTY);
                l->size++;
    ...
    /**
     * @brief Generates all bishop captures.
     *
     * @tparam side_to_move boolean indicating side to move
     * @param b pointer to the board representation
     * @param l pointer to the movelist
     */
    template <bool side_to_move> static void generate_bishop_cmoves(bitboard *b, movelist *l) {
        uint64_t bishops = b->bbs[BISHOP][side_to_move];
        unsigned long origin;
        unsigned long target;
        uint64_t bishop_attacks = 0ULL;
        ...
        while (bishops != 0ULL) {
            origin = BitScanForward64(bishops);
            bishop_attacks = attacks::get_bishop_attacks(origin, b->occupancy[2]) &
                             b->occupancy[!b->side_to_move];
            while (bishop_attacks != 0ULL) {
                target = BitScanForward64(bishop_attacks);
                capture_type = b->piece_type_from_index(target);
                l->moves[l->size].m = bit_move((uint8_t)origin, (uint8_t)target, bit_move::capture,
    ...
    /**
     * @brief Generates all quiet rook moves. (no captures)
     *
     * @tparam side_to_move boolean indicating side to move
     * @param b pointer to the board representation
     * @param l pointer to the movelist
     */
    template <bool side_to_move> static void generate_rook_qmoves(bitboard *b, movelist *l) {
        ...
        while (rooks != 0ULL) {
            ...
            while (rook_attacks != 0ULL) {
                ...
                l->moves[l->size].m =
                    bit_move((uint8_t)origin, (uint8_t)target, bit_move::quiet_move, ROOK, EMPTY);
                l->size++;
    ...
    /**
     * @brief Generates all rook captures.
     *
     * @tparam side_to_move boolean indicating side to move
     * @param b pointer to the board representation
     * @param l pointer to the movelist
     */
    template <bool side_to_move> static void generate_rook_cmoves(bitboard *b, movelist *l) {
        uint64_t rooks = b->bbs[ROOK][side_to_move];
        uint8_t origin;
        uint8_t target;
        uint64_t rook_attacks = 0ULL;
        ...
        while (rooks != 0ULL) {
            origin = BitScanForward64(rooks);
            rook_attacks = attacks::get_rook_attacks(origin, b->occupancy[2]) &
                           (b->occupancy[!b->side_to_move]);
    ...
    template <bool side_to_move> static void generate_king_qmoves(bitboard *b, movelist *l) {
        // if white has the move
        if (!side_to_move) {
            if ((b->castling_rights & b->w_kingside) &&
                !(b->occupancy[2] & w_kingside_castling_mask)) {
                l->moves[l->size].m = bit_move(E1, G1, bit_move::kingside_castle, KING, EMPTY);
                l->size++;
            }
            if ((b->castling_rights & b->w_queenside) &&
                !(b->occupancy[2] & w_queenside_castling_mask)) {
        ...
        // if black has the move
            ...
            if ((b->castling_rights & b->b_kingside) &&
                !(b->occupancy[2] & b_kingside_castling_mask)) {
                l->moves[l->size].m = bit_move(E8, G8, bit_move::kingside_castle, KING, EMPTY);
                l->size++;
            }
            if ((b->castling_rights & b->b_queenside) != 0 &&
                !(b->occupancy[2] & b_queenside_castling_mask)) {
        ...
        while (king_attacks != 0ULL) {
            ...
            l->moves[l->size].m =
                bit_move((uint8_t)origin, (uint8_t)target, bit_move::quiet_move, KING, EMPTY);
            l->size++;
